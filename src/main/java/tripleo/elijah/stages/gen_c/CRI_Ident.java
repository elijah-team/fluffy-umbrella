...
class CRI_Ident {
	...
	private @Nullable EvaNode _re_is_PropertyStatement(final @NotNull Consumer<CReference.Reference> addRef,
		...
		final EvaNode resolved1 = ite.getType().resolved();
		final int     code;
		if (resolved1 != null)
		...
		if (resolved_element != null) {
			...
			if (repo_element == null) {
				...
				if (resolved_element instanceof ClassStatement) {
					...
					skip[0] = false;
				} else if (resolved_element instanceof VariableStatement vresolvedElement) {
					//repo_element.set
					//addRef.accept(new CReference.Reference(resolvedElement.getName(), CReference.Ref.MEMBER/*??*/));

					item.setText(vresolvedElement.getName());
					if (!aCReference.refs.isEmpty()) {
		...
		if (repo_element != null) {
			...
			if (!skip[0]) {
				...
				case 1:
					...
					break;
				case 2:
					if ((resolved_element instanceof VariableStatement vresolvedElement)) {
						final String text2 = vresolvedElement.getName();
