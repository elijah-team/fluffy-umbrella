...
//			s.pop();
	...
	tripleo.elijah.util.Stupidity.println_err("979 During deduceProcedureCall " + pce);
		...
		if (!finished) {
			if (best != null) {
				...
				tripleo.elijah.util.Stupidity.println_err("992 " + best.getClass().getName());
...
//				return deduceExpression(vs.initialValue(), ctx); // infinite recursion
	...
	static @Nullable OS_Element lookup(@NotNull IExpression expression, @NotNull Context ctx, @NotNull DeduceTypes2 deduceTypes2) throws ResolveError {
		...
		case DOT_EXP:
			...
			return best3;
//		default:
//		tripleo.elijah.util.Stupidity.println_err("1242 "+expression);
//			throw new NotImplementedException();
		default:
