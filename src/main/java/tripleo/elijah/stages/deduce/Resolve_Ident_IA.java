...
//				assert idte.getStatus() != BaseTableEntry.Status.UNCHECKED;
			...
			tripleo.elijah.util.Stupidity.println_err("385 idte.resolveExpectation is null for "+idte);
			} else
		...
		if (pte.getFunctionInvocation() == null) {
			...
			if (setClassInvocation) {
				if (ci != null) {
					...
					tripleo.elijah.util.Stupidity.println_err("542 Null ClassInvocation");
			}
