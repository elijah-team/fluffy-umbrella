...
public class FluffyModuleImpl implements FluffyModule {
	...
	private static void faep_002(final @NotNull ClassStatement classStatement, final Consumer<ClassStatement> ccs) {
		final List<ClassItem>     x     = classStatement.findFunction("main");
		final Stream<FunctionDef> found = x.stream().filter(input -> {
			var input2 = (OS_Element2) input;
			return FluffyCompImpl.isMainClassEntryPoint(input2);
