 ...
 */
public abstract class BaseEvaFunction_1 extends AbstractDependencyTracker implements BaseEvaFunction {
	...
	public String getIdentIAPathNormal(final IdentIA ia2) {
		final List<InstructionArgument> s = BaseEvaFunction._getIdentIAPathList(ia2);
		...
		for (final InstructionArgument ia : s) {
			...
			if (ia instanceof IntegerIA) {
				...
				assert prte.expression.getExpression() instanceof ProcedureCallExpression;
				text = ((ProcedureCallExpression) prte.expression.getExpression()).printableString();
			} else
