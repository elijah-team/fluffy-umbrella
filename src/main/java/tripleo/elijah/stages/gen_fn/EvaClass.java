	...
	public void fixupUserClasses(final DeduceTypes2 aDeduceTypes2, final Context aContext) {
		for (final VarTableEntry varTableEntry : varTable) {
			varTableEntry.updatePotentialTypesCB = new VarTableEntry.UpdatePotentialTypesCB() {
				@Override
				public void call(final @NotNull EvaContainer aGeneratedContainer) {
...
//											xxxci = ci;
										for (final Map.@NotNull Entry<TypeName, OS_Type> entry : xxci.genericPart_.entrySet()) {
...
//					final Set<GenType> s = Collections.unmodifiableSet(set);
	...
	public String getName() {
		...
		if (ci.genericPart_ != null) {
			sb.append("[");
			final String joined = getNameHelper(ci.genericPart_.getMap());
			sb.append(joined);
			sb.append("]");
		}
		return sb.toString();
