...
public class VariableTableEntry extends BaseTableEntry1 implements Constructable, TableEntryIV, DeduceTypes2.ExpectationBase {
	...
	public void addPotentialType(final int instructionIndex, final TypeTableEntry tte) {
		...
		tripleo.elijah.util.Stupidity.println_err("62 addPotentialType while typeDeferred is already resolved "+this);//throw new AssertionError();
			return;
		}
		//
		if (!potentialTypes.containsKey(instructionIndex))
			...
			if (v.getAttached() == null) {
				...
				// May not be any
				//
//			tripleo.elijah.util.Stupidity.println_err("v.attached: " + v.attached);
//			tripleo.elijah.util.Stupidity.println_err("tte.attached: " + tte.attached);
				tripleo.elijah.util.Stupidity.println_out("72 WARNING two types at the same location.");
	...
	public void resolveType(final @NotNull GenType aGenType) {
		if (_resolveTypeCalled != null) { // TODO what a hack
			if (_resolveTypeCalled.resolved != null) {
				...
				tripleo.elijah.util.Stupidity.println_err(String.format("** 130 Attempting to replace %s with %s in %s", _resolveTypeCalled.asString(), aGenType.asString(), this));
//					throw new AssertionError();
		...
		tripleo.elijah.util.Stupidity.println_err("126 typeDeferred is resolved "+this);
		}
		_resolveTypeCalled = aGenType;
		typeDeferred.resolve(aGenType);
	}
