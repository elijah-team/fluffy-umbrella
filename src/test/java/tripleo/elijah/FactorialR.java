...
//		super(name);
	...
	void main2_el(final CompilerContext cctx, final @NotNull GenBuffer gbn) {
		...
		final @NotNull TypeRef   sys_int = new TypeRef(prelude, prelude, "SystemInteger", BuiltInTypes.SystemInteger.getCode());
		final @NotNull ModuleRef main2   = new ModuleRef("main2/main2.elijjah", 10000);
		final @NotNull TypeRef   main    = new TypeRef(main2, main2, "Main", 100);
				
		gbn.InitMod(cctx, "main2/main2.elijjah");
		final @NotNull ImportNode impn = new ImportNode("wprust.demo.fact");
		gbn.GenImportStmt(cctx, impn);
		
		final @NotNull ClassDeclNode cdn = new ClassDeclNode("Main", null,
				List_of(new Inherited("Arguments", false))); // gen inh code 
		cdn.GenClassDecl(cctx, gbn);
		
		final @NotNull MethHdrNode mhn = new MethHdrNode(null, cdn.type(), "main", null, 1000);
		mhn.GenMethHdr(cctx, gbn);
		mhn.BeginMeth(cctx, gbn);
		
		final @NotNull LocalDeclAgnNode ldan1 = new LocalDeclAgnNode("b1", ExpressionNodeBuilder.integer(3));
		gbn.GenLocalDeclAgn(cctx, ldan1);
		
		final @NotNull LocalDeclAgnNode ldan_a1 = new LocalDeclAgnNode("a1", sys_int,
		...
		final @NotNull TmpSSACtxNode latn = new TmpSSACtxNode(cctx);
		latn.GenLocalAgn(cctx, gbn);
		
		final @NotNull IfNode ifn = new IfNode(latn);
		ifn.BeginIfCtx(cctx, gbn);
		
		final @NotNull LocalAgnNode lan_b1 = new LocalAgnNode(
				ExpressionNodeBuilder.fncall("a1.toInt", null));
		lan_b1.GenLocalAgn(cctx, gbn);
		ifn.CloseIfCtx(cctx, gbn);
		
		final @NotNull LocalDeclAgnNode ldan_f1 = new LocalDeclAgnNode("f1",
		...
		final @NotNull SimpleFnCall sfc = new SimpleFnCall("print", List_of("f1"));
	...
	private List<LocalAgnTmpNode> convertToLocalAgnTmpNode(final List<@NotNull NumericExpression> list_of) {
		...
		final @Nullable List<LocalAgnTmpNode> r = null;
		throw new NotImplementedException();
//		return r;
	}

	void factorial_r(final @NotNull CompilerContext cctx, final @NotNull GenBuffer gbn) {
		final @NotNull ModuleRef prelude = new ModuleRef(null, -1);
		...
		final @NotNull CaseChoiceNode csn=new CaseChoiceNode(ExpressionNodeBuilder.integer(0), shn);
		BeginCaseChoice(cctx, csn, gbn);
		
//		ReturnAgnSimpleIntNode rasin=new ReturnAgnSimpleIntNode(ExpressionNodeBuilder.integer(1));
		final @NotNull ReturnAgnNode rasin=new ReturnAgnNode(ExpressionNodeBuilder.integer(1));
//		GenReturnAgnSimpleInt(cctx, rasin, gbn);
		GenReturnAgn(cctx, rasin, gbn);
		
		final @NotNull CloseCaseNode cccn1 = new CloseCaseNode(csn, ChoiceOptions.BREAK);
		CloseCaseChoice(cctx, cccn1, gbn);

		final @NotNull CaseChoiceNode csn2 = new CaseChoiceNode(cctx, ExpressionNodeBuilder.varref("n", shn, u64), shn);
		...
		final @NotNull TmpSSACtxNode tccssan3 = new TmpSSACtxNode(cctx);
		final @NotNull LocalAgnTmpNode latn3=new LocalAgnTmpNode(tccssan3, ExpressionNodeBuilder.binex(u64,
...
//		ReturnAgnSimpleIntNode rsin=new ReturnAgnSimpleIntNode(latn3); // TODO Not a simple int
		final @NotNull ReturnAgnNode rsin=new ReturnAgnNode(latn3); // TODO Not an expression!!!
...
*/
		...
		final @NotNull CloseCaseNode cccn2=new CloseCaseNode(csn2, ChoiceOptions.BREAK, true);
	...
	@Deprecated private void GenReturnAgnSimpleInt(final @NotNull CompilerContext cctx, final @NotNull ReturnAgnSimpleIntNode rasin, final @NotNull GenBuffer gbn) {
		final TextBuffer buf=gbn.moduleBufImpl(cctx.module());
		buf.append("vsr = ");
